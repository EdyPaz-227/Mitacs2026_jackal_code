...
class JoystickTrajectoryController(Node):
    ...
    def _joy_callback(self, message: Joy) -> None:
        ...
        if manual_now:
            ...
            # Triangle edges are not processed while L1/R1 are held, so a
            # release during manual override would leave triangle_is_down stuck
            # True and start return-to-home on its own after L1/R1 release.
            self.triangle_is_down = False
    ...
    def _start_return_home(self) -> None:
        # Consume the hold even when the guards below fail, otherwise the
        # control loop retries and warns at control_rate while Triangle is held.
